...
function setupInput() {
    ...
    // Game Rules Modal Setup
    const openGameRulesModal = () => {
        updateGameRulesUI();
        document.getElementById('game-rules-modal').style.display = 'block';
    };

    const btnGameRulesMenu = document.getElementById('btn-game-rules-menu');
    if (btnGameRulesMenu) {
        btnGameRulesMenu.addEventListener('click', openGameRulesModal);
    }
    const btnGameRulesSim = document.getElementById('btn-game-rules');
    if (btnGameRulesSim) {
        btnGameRulesSim.addEventListener('click', openGameRulesModal);
    }

    const btnToggleEventModal = document.getElementById('btn-toggle-event-modal');
    if (btnToggleEventModal) {
        btnToggleEventModal.addEventListener('click', () => {
            isEventModalEnabled = !isEventModalEnabled;
            updateGameRulesUI();
        });
    }

    const btnToggleSpecialAlliances = document.getElementById('btn-toggle-special-alliances');
    if (btnToggleSpecialAlliances) {
        btnToggleSpecialAlliances.addEventListener('click', () => {
...
function updateGameRulesUI() {
    const btnEventModal = document.getElementById('btn-toggle-event-modal');
    if (btnEventModal) {
        btnEventModal.innerText = isEventModalEnabled ? 'オン' : 'オフ';
        btnEventModal.style.background = isEventModalEnabled ? '#27ae60' : '#c0392b';
    }

    const btnSpecialAlliances = document.getElementById('btn-toggle-special-alliances');
